...
### Аутентификация
- **JWT** (stateless) — токен выдаётся на `/api/auth/login`, время жизни 1 час
- **BCrypt** — хеширование паролей
- Роли: `ADMIN`, `SERVICE`, `MANAGEMENT`, `MANUFACTURER`

### Rate Limiting
- Реализован через **Bucket4j** (token bucket алгоритм)
- Защищает `/api/auth/login` и `/api/auth/register` от brute-force и спам-регистрации
- Лимит: **5 запросов в минуту** на IP-адрес
- При превышении — HTTP `429 Too Many Requests` с заголовком `Retry-After`
- Автоматическая очистка неактивных бакетов каждые 60 секунд (TTL 10 минут)
